...
def _closest_pair(points_x, points_y):
    ...
    dist_left, l_pointX, l_pointY = _closest_pair(left_X, left_Y)
    dist_right, r_pointX, r_pointY = _closest_pair(right_X, right_Y)

    dist_smaller, pointX_smaller, pointY_smaller = (dist_left, l_pointX, l_pointY) if dist_left <= dist_right \
        else (dist_right, r_pointX, r_pointY)

    closest_Y = [x for x in points_y if
                 abs(x[0] - middle_X) <= dist_smaller]
    ...
    if len_y > 1:
        for i in range(len_y - 1):
            for j in range(i + 1, min(i + 7, len_y)):
                p1 = closest_Y[i]
                p2 = closest_Y[j]
                distY = distance(p1, p2)
                if distY < dist_smaller:
                    pointX_smaller = p1
                    pointY_smaller = p2
                    dist_smaller = distY

    return dist_smaller, pointX_smaller, pointY_smaller


def brute_force(points_x):
    n = len(points_x)
    # if n < 2:
    #     return None
    point1 = points_x[0]
    ...
    for i in range(n - 1):
        for j in range(i + 1, n):
            if i != 0 and j != 1:
                temp_dist = distance(points_x[i], points_x[j])
                if temp_dist < dist:
                    dist = temp_dist
                    point1 = points_x[i]
                    point2 = points_x[j]
...
def closest_pair(points):
    ...
    dist, pointX, pointY = _closest_pair(points_x, points_y)

    return dist, (pointX, pointY)


# arr = [(82, 66), (33, 62), (98, 19), (41, 25), (92, 94), (17, 29), (78, 30), (25, 92), (19, 82), (42, 31)]
